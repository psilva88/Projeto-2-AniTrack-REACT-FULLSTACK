...
function Login() {
  ...
  const { login, entrarVisitante } = useAuth();
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setErroApi(err.message || "Erro ao fazer login.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div style={{
      background: "linear-gradient(160deg, #080814 0%, #0d1628 50%, #0a1929 100%)",
      ...
      <div style={{ marginBottom: "28px", zIndex: 1 }}>
        <div style={{ display: "inline-block" }}>
          <img src="/images/logo.png" alt="AniTrack" style={{ height: "130px", width: "auto", display: "block" }} />
        </div>
      </div>
      ...
      <div style={{
        ...
        <h1 style={{ color: "#f1f5f9", fontSize: "22px", fontWeight: "700", fontFamily: fontDisplay, letterSpacing: "-0.3px", marginBottom: "6px", textAlign: "center" }}>
          Bem-vindo de volta
        </h1>
        <p style={{ color: "#475569", fontSize: "14px", fontFamily: font, textAlign: "center", marginBottom: "28px" }}>
          Entre na sua conta para acessar sua lista
        </p>

        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div>
            <label style={{ display: "block", color: "#64748b", fontSize: "12px", fontWeight: "600", fontFamily: font, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: "7px" }}>E-mail</label>
            <input style={inputStyle(!!erros.email)} name="email" type="email" value={form.email} onChange={handleChange} placeholder="seu@email.com" autoComplete="email" />
            {erros.email && <p style={{ color: "#ef4444", fontSize: "12px", marginTop: "5px", fontFamily: font }}>{erros.email}</p>}
          </div>

          <div>
            <label style={{ display: "block", color: "#64748b", fontSize: "12px", fontWeight: "600", fontFamily: font, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: "7px" }}>Senha</label>
            <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
              <input style={{ ...inputStyle(!!erros.senha), paddingRight: "44px" }} name="senha" type={showSenha ? "text" : "password"} value={form.senha} onChange={handleChange} placeholder="••••••••" autoComplete="current-password" />
          ...
          <button type="submit" disabled={loading} style={{ width: "100%", padding: "13px", borderRadius: "10px", border: "none", background: loading ? "#5b21b6" : "#7c3aed", color: "#fff", fontSize: "15px", fontWeight: "600", fontFamily: font, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", marginTop: "4px" }}>
            {loading ? (<><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"><animateTransform attributeName="transform" type="rotate" from="0 12 12" to="360 12 12" dur="0.8s" repeatCount="indefinite"/></path></svg>Entrando...</>) : "Entrar"}
          </button>
        </form>
        ...
        <button onClick={entrarComoVisitante} style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "#94a3b8", fontSize: "14px", fontWeight: "500", fontFamily: font, cursor: "pointer" }}>
          Continuar como visitante
        </button>

        <p style={{ textAlign: "center", color: "#475569", fontSize: "14px", fontFamily: font, marginTop: "22px" }}>
          Não tem uma conta?{" "}
          <Link to="/cadastro" style={{ color: "#a78bfa", fontWeight: "600", textDecoration: "none" }}>Criar conta</Link>
        </p>
      </div>
    </div>
  );
