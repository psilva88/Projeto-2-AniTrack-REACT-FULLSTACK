...
export function AnimeProvider({ children }) {
  ...
  async function updateEpisodios(id, delta) {
    ...
    // Status automático conforme os episódios:
    // 0 = Planejando | todos = Finalizado | meio = Assistindo
    // Exceção: se a pessoa marcou "Pausado" manualmente e ainda está no meio, respeita o Pausado.
    let novoStatus;
    if (novoVal === 0) {
      novoStatus = "Planejando";
    } else if (anime.totalEps > 0 && novoVal === anime.totalEps) {
      novoStatus = "Finalizado";
    } else {
      // está no meio: mantém "Pausado" se foi escolhido manualmente, senão "Assistindo"
      novoStatus = anime.status === "Pausado" ? "Pausado" : "Assistindo";
    }

    const finalizou = novoStatus === "Finalizado" && anime.status !== "Finalizado";
